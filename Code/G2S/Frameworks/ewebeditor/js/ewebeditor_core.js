...
var am = (function () {
    ...
    var mv = function () {
        ...
        kP = "<html><head>" + "<link href='" + EWEB.cd + "/language/" + lang.bF + ".css' type='text/css' rel='stylesheet'>" + "<link href='" + EWEB.cd + "/skin/" + config.Skin + "/menuarea.css' type='text/css' rel='stylesheet'>" + "</head>" + "<body scroll='no'>";
        if (F.as) {
...
var EWEB = {
    ...
    Init: function () {
        ...
        this.hW = document.location.protocol.toLowerCase() + "//" + aA.H + aA.Hw;
...
function Dj() {
    ...
    if (xv) {
        ...
        var Ee = parseInt(R.ce(xv, "border-top-width"));
        var EF = parseInt(R.ce(xv, "border-bottom-width"));
        var Dv = parseInt(R.ce(xv, "border-left-width"));
        var EG = parseInt(R.ce(xv, "border-right-width"));
        xv.style.width = (be.clientWidth - Dv - EG) + 'px';
        xv.style.height = (be.clientHeight - Ee - EF) + 'px';
        xv.style.height = '100%';
        DN();
    } else {
...
function localUpload() {
    ...
    var fh = getHTML();
    FT.kI.LocalUpload(fh);
    window.setTimeout("Ec('')", 100);
};

function Ec(Ef) {
    if (FT.kI.Status != "ok") {
        window.setTimeout("Ec('" + Ef + "')", 100);
        return;
    }
    if (FT.Fi()) {
        $("divProcessing").style.display = "none";
    ...
    addUploadFiles(FT.kI.OriginalFiles, FT.kI.SavedFiles);
    var fh = FT.kI.Body;
    if (Ef == "dB") {
        insertHTML(fh);
    } else {
...
function getText() {
    ...
    if (EWEB.ae == "TEXT") {
        ...
        bE = _Html2Text(getHTML());
    }
    return bE;
};

function _Html2Text(V) {
...
function oX() {
    ...
    if (config.AutoDetectPaste == "1") {
        ...
        var an = FT.kI.GetClipboard("flag");
        var fB = an.split("|");
        if (fB[5] == "1") {
            kF();
            return;
            ...
            var fh = FT.kI.GetClipboard("wps");
            FT.kI.LocalUpload(fh);
            window.setTimeout("Ec('dB')", 100);
            return;
        } else if (fB[5] == "4") {
            if (FT.FD()) {
                return;
            }
            var fh = FT.kI.GetClipboard("et");
            FT.kI.LocalUpload(fh);
            window.setTimeout("Ec('dB')", 100);
            return;
            ...
            var fh = FT.kI.GetClipboard("html");
            insertHTML(fh);
            return;
        } else if (fB[1] == "1") {
            var fh = FM(FT.kI.GetClipboard("text"));
            insertHTML(fh);
            return;
        } else {
            return;
